...
from src.utils.validate_data_new import validate_data_churn_data_new

def main(args):

    project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    mlflow_uri = args.mlflow_uri or f"sqlite:///{project_root}/mlruns/mlflow.db"
    ...
    with mlflow.start_run():
        ...
        is_valid, failed = validate_data_churn_data_new(df)
        ...
        processed_path = os.path.join(project_root, "data", "processed", "bank_churn_processed.csv")
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(description="Run bank churn pipeline with XGBoost + MLflow")
    p.add_argument("--input", type=str, required=True,
                   help="path to CSV (e.g., data/Customer-Churn-Records.csv)")
    p.add_argument("--target", type=str, default="Exited")
    p.add_argument("--threshold", type=float, default=0.3)
    p.add_argument("--test_size", type=float, default=0.2)
    p.add_argument("--experiment", type=str, default="Bank Churn - XGBoost")
